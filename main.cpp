...
using namespace std;
bool isInMap(const unordered_map<int, vector<string>> &hashMap, int key){
...
struct RollingHash{
    ...
    bool isInHashMap(string word){
        for (std::unordered_map<int, vector<string>>::iterator it = hashMap.begin(); it != hashMap.end(); it ++ ){
...
vector<string> split(string str, vector<char> divList, int minlength = -1, int wordsLimit = -1){
    vector<string> result;

    string currWord = "";
    for(int i = 0; i < str.size(); i ++){
        if(vectorContains(divList, str[i])){
            if(minlength == -1 || minlength <= currWord.size())
                result.push_back(currWord);
            currWord = "";
            if(wordsLimit != -1 && result.size() >= wordsLimit){
                break;
            }
            continue;
        }else{
...
int main() {
    ...
    for(int i = 0; i < n; i ++){
        getline(cin, line);
        vector<string> words = split(line, {',','.', '\"', '\'', ';', ':','!','?', ' '}, 5, 200);
